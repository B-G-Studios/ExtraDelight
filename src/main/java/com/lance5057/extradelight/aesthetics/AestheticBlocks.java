...
import net.minecraftforge.neoforge.client.model.generators.MultiPartBlockStateBuilder;
				...
				bsp.horizontalBlock(WREATHS.get(WOOD.values().length + j - 2).get(),
						bsp.models()
								.withExistingParent(EXTRA_LEAVES.values()[j].toString() + "_wreath_block",
										bsp.modLoc("block/wreath"))
								.texture("all",
										new ResourceLocation(ExtraDelight.MOD_ID, "block/crops/fruit/" + EXTRA_LEAVES.values()[j].toString() + "/"
	...
	public static void itemModel(ItemModelProvider tmp) {
		for (int i = 0; i < WOOD.values().length; i++) {
			...
			if (WOOD.values()[i].toString() == "cinnamon")
				...
				tmp.getBuilder(COUNTER_CABINET_ITEMS.get(i).getId().getPath())
						.parent(new ModelFile.UncheckedModelFile(new ResourceLocation("farmersdelight", "block/" + WOOD.values()[i].toString() + "_cabinet")));
		...
		for (int i = 0; i < DyeColor.values().length; i++) {
			tmp.getBuilder(WALLPAPER_ITEMS.get(i).getId().getPath())
					.parent(new ModelFile.UncheckedModelFile(new ResourceLocation(ExtraDelight.MOD_ID, "block/" + BuiltInRegistries.BLOCK
	...
	public static void Recipes(RecipeOutput consumer) {
		...
		for (int i = 0; i < DyeColor.values().length; i++) {
			...
			ShapelessRecipeBuilder.shapeless(RecipeCategory.DECORATIONS, WALLPAPER_ITEMS.get(dye.ordinal()).get(), 4)
					...
					.requires(ItemTags.create(new ResourceLocation("c", "dyes/" + dye)))
			...
			ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, BOW_ITEMS.get(dye.ordinal()).get(), 1).pattern(" w ")
					...
					.define('w',
							BuiltInRegistries.ITEM
									.get(new ResourceLocation("minecraft", dye + "_wool")))
			...
			ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, GINGHAM_CARPET_ITEMS.get(dye.ordinal()).get(), 4)
					...
					.define('c',
							BuiltInRegistries.ITEM
									.get(new ResourceLocation("minecraft", dye + "_carpet")))
	...
	static void moldRecipe(RecipeOutput consumer, Item slab, WOOD name, int add) {
		for (int j = 0; j < 16; j++) {
			...
			ShapelessRecipeBuilder.shapeless(RecipeCategory.DECORATIONS, MOLDED_WALLPAPER_ITEMS.get(j + add).get(), 4)
					.requires(Items.PAPER, 4)
					.requires(ItemTags.create(new ResourceLocation("c", "dyes/" + d))).requires(slab)
